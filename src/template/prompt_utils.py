...
def build_verify_prompt_CoT(question: str, schema: str, sql: str):
    system_prompt = get_system_prompt(template_file_name="zero_shot_en_CoT.md")
    query = get_query_template(template_file_name="verify_query_CoT.md")
    query = query.format(question=question, schema=schema, sql=sql)
    return [
        {
            'role': 'system',
...
# def build_verify_prompt_Confidence(question: str, schema: str, sql: str):
#     system_prompt = get_system_prompt(template_file_name="zero_shot_en_Confidence.md")
#     query = get_query_template(template_file_name="verify_query_Confidence.md")
#     query = query.format(question=question, schema=schema)#, sql=sql
#     return [
...
def build_verify_prompt_Confidence(question: str, schema: str, sql: str):
    system_prompt = get_system_prompt(template_file_name="zero_shot_en_Confidence.md")
    query = get_query_template(template_file_name="verify_query_Confidence.md")
    query = query.format(question=question, schema=schema)
    return [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": query},
