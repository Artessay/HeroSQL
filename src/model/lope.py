...
class Lope(nn.Module):
    ...
    def encode_graph(self, graph_batch: Batch):
        property_embedding = self.encode_text(
            graph_batch.graph_attributes_input_ids, 
            graph_batch.graph_attributes_attention_mask
        )
        
        return self.graph_encoder(property_embedding, graph_batch.edge_index, graph_batch.batch)
...
if __name__ == "__main__":
    ...
    _, _, test_loader = create_dataloader("bird", batch_size=3, mode="test", method_name="lope", embedding_model_name=embedding_model_name)
